...
import { borrowAny, borrowedAny, repayAny } from "../../../../helpers/treasuryMethods";
  ...
  const checkTokenAllowance = async (currentToken: UsableContract) => {
    if (currentToken && currentToken.contract && currentToken.decimal) {
      ...
      if (allowance.gt(anyToApprove)) {
        setButtonStatus({ ...buttonStatus, disable: false, repay: true });
    ...
    <div className={checkContent("repay")}>
      <div className={css(styles.mintInputWrapper)}>
        <div className={css(styles.mintTitle)}>
          <span>Available: {balance} {selectedToken?.symbol || ""}</span>
        ...
        <div className={css(styles.mintInnerWrap)}>
          ...
          <div className={css(styles.maxBtn)} onClick={handleMaximum}>Max</div>
          {currentAny?.name ? (
      ...
      <div className={css(styles.subTitle)}>Borrowed Amount: {borrowedAmount} Any</div>
